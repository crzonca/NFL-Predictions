...
class PlayoffPredictor:
    ...
    def get_remaining_win_probs(self, team_name, use_bt=True):
        ...
        if use_bt:
            ...
            for remaining_opponent in remaining_opponents:
                win, tie, loss = bets.get_spread_chance(team_name, remaining_opponent, 0)
                win_chance = win / (1 - tie)
                win_probs.append(win_chance)
    ...
    def print_full_playoff_chances(self, afc_playoff_teams, nfc_playoff_teams, playoff_chances):
        with open('resources/config.json', 'r') as f:
            config = json.load(f)
        ...
        # Print the table
        with open(config.get('output_locations').get('playoffs'), 'w') as f:
            f.write('Playoff chances assuming the following seeding:\n')
            f.write('AFC: ' + ' '.join([str(seed) + '. ' + team.ljust(self.config.get('justify_width'))
                                        for seed, team in zip(range(1, 8), afc_playoff_teams)]) + '\n')
            f.write('NFC: ' + ' '.join([str(seed) + '. ' + team.ljust(self.config.get('justify_width'))
    ...
    def win_conference_title_chance(self, playoff_teams):
        ...
        wc1 = Bracket('Wildcard 1', self.team_df, self.individual_df, self.graph, self.gen_poisson_model,
                      Bracket(seed2, self.team_df, self.individual_df, self.graph, self.gen_poisson_model, bt=bt2),
                      Bracket(seed7, self.team_df, self.individual_df, self.graph, self.gen_poisson_model, bt=bt7),
                      series_length=1)
        wc2 = Bracket('Wildcard 2', self.team_df, self.individual_df, self.graph, self.gen_poisson_model,
                      Bracket(seed3, self.team_df, self.individual_df, self.graph, self.gen_poisson_model, bt=bt3),
                      Bracket(seed6, self.team_df, self.individual_df, self.graph, self.gen_poisson_model, bt=bt6),
                      series_length=1)
        wc3 = Bracket('Wildcard 3', self.team_df, self.individual_df, self.graph, self.gen_poisson_model,
                      Bracket(seed4, self.team_df, self.individual_df, self.graph, self.gen_poisson_model, bt=bt4),
                      Bracket(seed5, self.team_df, self.individual_df, self.graph, self.gen_poisson_model, bt=bt5),
                      series_length=1)
        ...
        for wc_winner1, wc_winner2, wc_winner3 in itertools.product(*[(seed2, seed7), (seed3, seed6), (seed4, seed5)]):
            ...
            other_seeds = list({wc_winner1, wc_winner2, wc_winner3} - {lowest_seed})
            other_seed1, other_seed2 = other_seeds
            other_bt1 = bt_mapping.get(other_seed1)
            other_bt2 = bt_mapping.get(other_seed2)

            div1 = Bracket('Divisional 1', self.team_df, self.individual_df, self.graph, self.gen_poisson_model,
                           Bracket(seed1, self.team_df, self.individual_df, self.graph, self.gen_poisson_model, bt=bt1),
                           Bracket(lowest_seed, self.team_df, self.individual_df, self.graph, self.gen_poisson_model, bt=lowest_bt),
                           series_length=1)
            div2 = Bracket('Divisional 2', self.team_df, self.individual_df, self.graph, self.gen_poisson_model,
                           Bracket(other_seed1, self.team_df, self.individual_df, self.graph, self.gen_poisson_model, bt=other_bt1),
                           Bracket(other_seed2, self.team_df, self.individual_df, self.graph, self.gen_poisson_model, bt=other_bt2),
                           series_length=1)

            conf = Bracket('Conference',  self.team_df, self.individual_df, self.graph, self.gen_poisson_model, div1, div2, series_length=1)
    ...
    def get_win_super_bowl_chances(self, afc_teams, nfc_teams):
        ...
        for afc_winner, nfc_winner in itertools.product(*[afc_teams, nfc_teams]):
            ...
            sb = Bracket('Superbowl', self.team_df, self.individual_df, self.graph, self.gen_poisson_model,
                         Bracket(afc_winner, self.team_df, self.individual_df, self.graph, self.gen_poisson_model, bt=afc_bt),
                         Bracket(nfc_winner, self.team_df, self.individual_df, self.graph, self.gen_poisson_model, bt=nfc_bt),
                         series_length=1)
    ...
    def get_seeding_chance(self, team, ordering, seed):
        other_opponents = set(ordering) - {team}

        total_wins_chances = self.get_total_wins_chances(team)
        opp_wins_chances = {opp: self.get_total_wins_chances(opp) for opp in other_opponents}
        tiebreak_opponents = {opp: self.get_tiebreak(team, opp, divisional=False) for opp in other_opponents}
        ...
        for win_total, chance in total_wins_chances.items():
            ...
            # The teams chance to have n or fewer teams finish better than them
            pb = PoiBin(all_opponent_chances)

            num_better = len(other_opponents) - len(ordering[seed:])
            seed_finish_chance = pb.cdf(num_better)

            # Times the teams chance to get that number of wins
            win_chances.append(chance * seed_finish_chance)

        return sum(win_chances)

    def get_ordering_chance(self, ordering):
        divisional_winners = ordering[:4]
        wildcards = ordering[4:]

        # 1 seed: Division winner chance * chance better than 2, 3 & 4 seed
        # 2 seed: Division winner chance * chance better than 3 & 4 seed
        # 3 seed: Division winner chance * chance better than 4 seed
        # 4 seed: Division winner chance
        divisional_seed_chances = list()
        for index, team in enumerate(divisional_winners):
            seed_chance = self.get_seeding_chance(team, divisional_winners, index + 1)
            div_winner_chance = self.team_df.at[team, 'Win Division']
            divisional_seed_chances.append(div_winner_chance * seed_chance)

        divisional_seed_chance = np.prod(divisional_seed_chances)

        # 5 seed: Wildcard chance * chance better than 6 & 7 seed
        # 6 seed: Wildcard chance * chance better than 7 seed
        # 7 seed: Wildcard chance
        wildcard_seed_chances = list()
        for index, team in enumerate(wildcards):
            seed_chance = self.get_seeding_chance(team, wildcards, index + 1)
            wildcard_chance = self.team_df.at[team, 'Make Wild Card']
            wildcard_seed_chances.append(wildcard_chance * seed_chance)

        wildcard_seed_chance = np.prod(wildcard_seed_chances)

        return divisional_seed_chance * wildcard_seed_chance
